...
export class ModularCore {
 ...
 updateActive(){
  const active=new Set(['bridge','tools']);for(const f of ['moog','buchla','arp','ems','euro','fx'])if(this.target[f+'.level']>0||this.params[f+'.level']>.0001)active.add(f);
  for(let pass=0;pass<Object.keys(families).length;pass++){
   for(const [dest,cable]of Object.entries(this.connections))if(active.has(ports[dest].family)&&!cable.blocked)active.add(ports[cable.source].family);
 ...
 tick(mic=0){
  ...
  s['buchla.function']=this.benv.tick(bg,p['buchla.attack'],p['buchla.decay'],0,p['buchla.decay'],rate,this.keyboardRetrigger('buchla.gate')&&p['buchla.cycle']<.5)*10;
  ...
  if(this.active.arp){this.arp.target.octave=p['arp.octave']+Math.round(p.octave);this.arp.target.bend=p.bend;const av=this.arp.tick(mic);this.arpLeft=av[0];this.arpRight=av[1];for(const id of this.arpSignalKeys)s[id]=this.arp.signals[id.slice(4)]||0;s['arp.out']=(av[0]+av[1])*.7071;}
  ...
  if(this.active.euro){const ehz=this.frequency(this.input('euro.pitch'),p['euro.tune']);this.oscs[8].tick(ehz,rate);const ee=this.eenv.tick(this.input('euro.gate')>2,.002,p['euro.decay'],.15,p['euro.decay'],rate,this.keyboardRetrigger('euro.gate'));const timbre=clamp(p['euro.timbre']+this.input('euro.timbreIn')*.1,0,1),t=this.oscs[8].phase;let voice;

...
const dividerOutputs=[[2,'tools.div2'],[4,'tools.div4'],[8,'tools.div8']],vcaChannels=['A','B'].map(c=>({input:'tools.vca'+c+'In',cv:'tools.vca'+c+'CV',output:'tools.vca'+c,gain:'tools.gain'+c}));
...
export class Utilities {
 ...
 tick(p,s,input,reset=false){
  ...
  for(const [d,id] of dividerOutputs)s[id]=this.count>=0&&this.count%d===0&&this.clock.high?5:0;
  ...
  for(const channel of vcaChannels)s[channel.output]=clamp(input(channel.input)*clamp(p[channel.gain]+input(channel.cv)*.1,0,1),-20,20);
  const cv=input('tools.quantizeIn'),scale=Math.round(p['tools.scale']),root=Math.round(p['tools.root']);if(cv!==this.quantizeCV||scale!==this.scale||root!==this.root){this.quantizeCV=cv;this.scale=scale;this.root=root;this.quantized=quantize(cv,scale,root);}s['tools.quantized']=this.quantized;

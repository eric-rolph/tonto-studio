...
export function packSession(app,patch,tape){
 const parts=[],settings={reverse:!!tape.reverse,loop:!!tape.loop};for(const key of Object.keys(settingLimits))settings[key]=tape[key];
 ...
 const takes=tape.takes.map(t=>({name:t.name,wet:buffer(t.wet),dry:buffer(t.dry),wetGain:t.wetGain,dryGain:t.dryGain,offset:t.offset,rate:t.rate,muted:!!t.muted,reverse:!!t.reverse}));
 const metadata=encoder.encode(JSON.stringify({version:1,app,patch,settings,takes})),header=new Uint8Array(12);header.set(encoder.encode(magic));new DataView(header.buffer).setUint32(8,metadata.length,true);
...
export async function unpackSession(file,app){
 ...
 const length=new DataView(header).getUint32(8,true);if(length>1000000||length<2||12+length>file.size)throw new Error('Invalid session header.');
 ...
 const takes=[];for(const t of metadata.takes)takes.push({id:crypto.randomUUID(),name:String(t.name||'Take').slice(0,120),wet:await read(t.wet),dry:await read(t.dry),wetGain:bounded(t.wetGain,0,1.5,1),dryGain:bounded(t.dryGain,0,1.5,0),rate:bounded(t.rate,.25,4,1),offset:bounded(t.offset,0,60,0),muted:t.muted===true,reverse:t.reverse===true});
 const settings={reverse:metadata.settings?.reverse===true,loop:metadata.settings?.loop===true};for(const [key,[min,max]]of Object.entries(settingLimits))settings[key]=bounded(metadata.settings?.[key],min,max,key==='speed'?1:0);
...
export function setupSessions({app,engine,tape,getPatch,validatePatch,loadPatch,status}){
 ...
 save.onclick=()=>run(async()=>{await tape.stopRecord();download(packSession(app,getPatch(),tape),app+'.synthsession');status('Session saved with the patch, tape settings and full-resolution recordings.');});
 input.onchange=()=>run(async()=>{
  const file=input.files[0];if(!file)return;const session=await unpackSession(file,app),patch=validatePatch(session.patch);
  ...
  const takes=session.takes.map(t=>({...t,wet:convert(t.wet),dry:convert(t.dry)}));await tape.stopRecord();tape.stop();loadPatch(patch);tape.takes=takes;Object.assign(tape,session.settings);tape.changed();

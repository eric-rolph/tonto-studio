...
export class Tape extends EventTarget {
  constructor(engine){super();this.engine=engine;this.takes=[];this.speed=1;this.saturation=.15;this.wow=.002;this.flutter=.001;this.reverse=false;this.loop=false;this.playing=false;this.recording=false;this.nodes=[];this.playGeneration=0;this.exportFormat=24;this.dither=true;this.loopBars=0;}
  ...
  startPrepared(when,seconds=0,linked=false){const run=++this.playGeneration;this.linked=linked;this.baseTempo=this.engine.params.tempo;this.playing=true;this.playStart=when;this.position=seconds;this.player.port.postMessage({type:'start',run,frame:Math.round(when*this.engine.ctx.sampleRate),seconds,linked});this.changed();}
  ...
  stop(){const run=++this.playGeneration;this.player?.port.postMessage({type:'stop',run});this.playing=false;this.changed();}

...
export class Tape extends EventTarget {
  constructor(engine){super();this.engine=engine;this.takes=[];this.speed=1;this.saturation=.15;this.wow=.002;this.flutter=.001;this.reverse=false;this.loop=false;this.playing=false;this.recording=false;this.nodes=[];this.exportFormat=24;this.dither=true;this.loopBars=0;}
  ...
  async initialize(){await this.engine.start();if(this.capture)return;const ctx=this.engine.ctx;
    await ctx.audioWorklet.addModule('/tape-core.js');this.player=new AudioWorkletNode(ctx,'studio-tape-player',{numberOfInputs:0,numberOfOutputs:1,outputChannelCount:[2]});this.player.connect(this.engine.bus);this.player.port.onmessage=({data:m})=>{if(m.ready){this.preparations?.get(m.ready)?.();this.preparations?.delete(m.ready);return;}this.position=m.position;this.cycle=m.cycle;if(this.playing&&!m.running){this.playing=false;this.changed();}};
  ...
  startPrepared(when,seconds=0,linked=false){this.linked=linked;this.baseTempo=this.engine.params.tempo;this.playing=true;this.playStart=when;this.position=seconds;this.player.port.postMessage({type:'start',frame:Math.round(when*this.engine.ctx.sampleRate),seconds,linked});this.changed();}
  async play({when,seconds=0}={}){await this.prepare();this.startPrepared(when??this.engine.ctx.currentTime+.03,seconds);}

  checkEnded(){if(this.playing&&!this.takes.some(t=>!t.muted))this.stop();}
  stop(){this.player?.port.postMessage({type:'stop'});this.playing=false;this.changed();}

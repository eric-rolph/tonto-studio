...
export class Engine extends EventTarget{
 ...
 async initialize(){
  ...
  this.synth=new AudioWorkletNode(ctx,'tonto-console',{numberOfInputs:1,numberOfOutputs:3,outputChannelCount:[2,1,1]});
 ...
 on(note,velocity=1,id=note){this.notes.delete(id);this.notes.set(id,{note,velocity});this.released.delete(id);this.trigger();this.dispatchEvent(new CustomEvent('noteaction',{detail:{on:true,note,velocity,id}}));this.dispatchEvent(new Event('notes'));}
 off(id){if(!this.notes.has(id))return;if(this.sustain){this.released.add(id);return;}this.notes.delete(id);this.released.delete(id);this.trigger(false);this.dispatchEvent(new CustomEvent('noteaction',{detail:{on:false,id}}));this.dispatchEvent(new Event('notes'));}
 ...
 bindMidi(){
    ...
    this.boundMidi=inputs;for(const input of inputs)input.onmidimessage=({data})=>{const [status,n,v]=data,type=status&240,id=`midi-${input.id}-${status&15}-${n}`;if(type===144&&v>0)this.on(n,v/127,id);if(type===128||(type===144&&!v))this.off(id);if(type===224)this.set('bend',(((v<<7)|n)-8192)/8192*2);if(type===176){if(n===64)this.sustainPedal(v>=64,`midi-${input.id}-${status&15}`);if(n===120||n===123)this.panic();const useDefault=this.dispatchEvent(new CustomEvent('midicc',{cancelable:true,detail:{device:input.id,name:input.name,channel:status&15,cc:n,value:v}}));if(useDefault){if(n===1||n===11)this.set('expression',v/127);if(n===7)this.set('master',v/127);}this.dispatchEvent(new Event('control'));}};this.dispatchEvent(new CustomEvent('midistate',{detail:inputs.map(i=>i.name)}));}
}

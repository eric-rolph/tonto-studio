...
test('MIDI mappings honor logarithmic ranges and discrete switches and selectors',()=>{
 assert.equal(controllerValue('moog.cutoff',0),controls['moog.cutoff'].min);assert.equal(controllerValue('moog.cutoff',127),controls['moog.cutoff'].max);const c=controls['moog.cutoff'];assert.ok(Math.abs(controllerValue('moog.cutoff',63.5)-Math.sqrt(c.min*c.max))<.001);assert.equal(controllerValue('euro.freeze',63),0);assert.equal(controllerValue('euro.freeze',64),1);assert.equal(controllerValue('mains',64),60);assert.equal(controllerValue('fx.monitor',127),4);
});
